...
# ─────────────────────────────────────────────────────────────
# V3.10 · 东财统一限流入口 — 对标 a-stock-data 的 em_get()
# 所有直连东财的 HTTP 请求必须走此入口而非裸 requests.get
# ─────────────────────────────────────────────────────────────
_EM_SESSION: requests.Session | None = None
_EM_LAST_CALL: float = 0.0
_EM_MIN_INTERVAL: float = 1.0  # 最小间隔秒数
_EM_LOCK = __import__("threading").Lock()


def em_get(url: str, params: dict | None = None, timeout: float = 12,
           headers: dict | None = None, **kwargs) -> requests.Response | None:
    """东财统一限流 GET 请求。

    自动做到：
    - 串行（全局锁，绝不对东财开并发）
    - ≥1s 间隔 + 随机抖动 (±0.3s)
    - 复用 HTTP 会话（Keep-Alive）
    - 默认 UA + Referer
    - V3.10 新增：对齐 a-stock-data V3.2 防封策略
    """
    global _EM_SESSION, _EM_LAST_CALL
    import random
    ...
    with _EM_LOCK:
        ...
        try:
            r = _EM_SESSION.get(url, params=params, timeout=timeout, headers=req_headers, **kwargs)
...
def _fetch_basic_a(ti: TickerInfo) -> dict:
    ...
    # V3.10 · PRIMARY: 腾讯 qt.gtimg.cn — 不封IP、零key、稳定，对标 a-stock-data 策略
    # 优先于 akshare/XueQiu 获取行情核心字段（price/PE/PB/市值）
    # 腾讯仅提供 quote 数据，不带 F10/公司资料，所以 akshare 仍需跑补充字段
    _tencent_has_price = False
    if requests:
        try:
            prefix = "sh" if ti.full.endswith("SH") else "sz"
            url = f"https://qt.gtimg.cn/q={prefix}{ti.code}"
            ...
            if "~" in text:
                ...
                if len(parts) > 45:
                    ...
                    out.update({
                        ...
                        "change_pct": round(chg_pct, 2) if price else out.get("change_pct"),
                        "pe_ttm": pe_ttm or out.get("pe_ttm"),
                        "pb": pb_val or out.get("pb"),
                        "market_cap": f"{total_mcap_yi}亿" if total_mcap_yi else out.get("market_cap"),
                        "market_cap_raw": total_mcap_yi * 1e8 if total_mcap_yi else out.get("market_cap_raw"),
                        "circulating_cap": f"{circ_mcap_yi}亿" if circ_mcap_yi else out.get("circulating_cap"),
                    })
                    if price:
                        _tencent_has_price = True
                    out["_fallback_snap"] = "tencent-qt-primary"
        except Exception as e:
            out["_tencent_primary_err"] = str(e)[:80]

    # PRIMARY: stock_individual_basic_info_xq (XueQiu backend — 补充公司资料/F10)
    if ak:
        try:
            df = _retry(lambda: ak.stock_individual_basic_info_xq(symbol=xq_symbol), attempts=2, sleep=1.5)
    ...
    # PRIMARY (ENRICHMENT): stock_individual_spot_xq (XueQiu realtime quote — 补充52周/换手率/总股本等)
    # 如果腾讯已拿到价格，降级为 2 次尝试补充字段；若腾讯失败，走原 4 次重试
    _xq_attempts = 2 if _tencent_has_price else 4
    _xq_sleep = 1.0 if _tencent_has_price else 2.0
    try:
        df = _retry(lambda: ak.stock_individual_spot_xq(symbol=xq_symbol), attempts=_xq_attempts, sleep=_xq_sleep)
        if df is not None and not df.empty:
            info = dict(zip(df["item"], df["value"]))
    ...
    # Fallback 1: direct push2 HTTP for single ticker (bypass spot_em bulk，走 em_get 限流)
    if requests and (not out.get("price") or _needs_pe_or_mcap()):
        try:
            ...
            if r is None or r.status_code != 200:
                raise RuntimeError("em_get failed")
...
def _kline_a_share_chain(ti: TickerInfo, period: str, start: str, adjust: str) -> list[dict]:
    ...
    # V3.10 · 优先级重排：不封IP的源优先（新浪/腾讯/baostock），东财降为兜底
    # ── 1. 新浪直连 HTTP（不封IP，零鉴权）
    if requests:
        try:
    ...
    if requests:
        try:
            ...
            r = em_get(url, params=params, timeout=12)
            if r is None or r.status_code != 200:
                raise RuntimeError("em_get failed")
            ...
            errors.append(f"em-direct-throttled: {e}")
...
def _fetch_financials_impl(ti: TickerInfo) -> dict:
    if ti.market == "A" and ak:
        try:
            ...
            # FALLBACK: 新浪直连财报三表（quotes.sina.cn — 不封IP，零鉴权）
            if requests:
                try:
                    prefix = "sh" if ti.full.endswith("SH") else "sz"
                    paper_code = f"{prefix}{ti.code}"
                    result = {"sina_fallback": True, "error": str(e), "statements": {}}
                    for rtype, rname in [("lrb", "利润表"), ("fzb", "资产负债表"), ("llb", "现金流量表")]:
                        url = "https://quotes.sina.cn/cn/api/openapi.php/CompanyFinanceService.getFinanceReport2022"
                        params = {"paperCode": paper_code, "source": rtype, "type": "0", "page": "1", "num": "8"}
                        r = requests.get(url, params=params, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
                        report_list = (r.json() or {}).get("result", {}).get("data", {}).get("report_list", {}) or {}
                        rows = []
                        for period in sorted(report_list.keys(), reverse=True)[:8]:
                            ...
                            rows.append(rec)
                        result["statements"][rname] = rows
                    return result
                except Exception as e2:
                    return {"error": f"akshare: {e}; sina also failed: {e2}"}
            return {"error": str(e)}
